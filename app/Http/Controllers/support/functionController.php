...
class functionController extends Controller
{
    ...
    public static function imageCreateAvif(Request $request,$path,$name)
    {
        $image=$request->File($name);
        $name=rand(100000000,999999999).".avif";
        $image2=\Intervention\Image\Facades\Image::make($image->getRealPath());
        $image2->save(public_path("imageAvif/$path/".$name))->stream("avif",100);
        return $name;
    }
    ...
    public static function imageCreateAvifGallery($path,$realPath)
    {
        $name=rand(100000000,999999999).".avif";
        $image2=\Intervention\Image\Facades\Image::make($realPath);
        $image2->save(public_path("imageAvif/$path/".$name))->stream("avif",100);
